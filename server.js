...
const getPostUrls = async (siteUrl) => {
    try {
        ...
        const { data } = await axios.get(`${siteUrl}/wp-json/wp/v2/posts?per_page=100`);
        ...
        return data.map((post) => {
            ...
            const titleParts = firstTwoWords.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1));

            // Join the title parts and remove 'Mod Apk'
            const title = titleParts.join(' ').replace(/Mod Apk/i, '').replace(/Download/i, '');
        ...
        for (const appUrl of appUrls) {
            try {
                ...
                if (searchResults.length > 0) {
                    ...
                    const apkVersion = $("span.spec-cont[itemprop='softwareVersion']").text().trim();
                    ...
                    const versionUpdateStatus = appDetails.version === cleanedApkVersion ? 'No Update Needed' : 'Need Update';
                    ...
                    //console.log('App data saved:', savedAppData);
                }
            } catch (error) {
                console.error('An error occurred:', error);
            }
